...
def run_handshake_trial(broker_host: str, broker_port: int, timeout_sec: int = 5):
    client = mqtt.Client()
    client.socket_timeout = timeout_sec

    # TLS setup for ECC baseline
    ...
    try:
        ...
        # send a few messages so packet loss/jitter actually matters
        for i in range(10):
            client.publish("test/topic", f"msg {i}")
            time.sleep(0.01)
